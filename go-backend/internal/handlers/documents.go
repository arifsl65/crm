...
func (h *DocumentHandler) List(c *gin.Context) {
	...
	query.WriteString(`
		SELECT d.id, d.tenant_id, d.client_id, d.service_id, d.uploaded_by, d.type_id,
		       ...
		       c.company_name as client_name, dt.name as type_name, COALESCE(u.first_name || ' ' || u.last_name, u.first_name, u.last_name, '') as uploaded_by_name
		FROM documents d
...
func (h *DocumentHandler) GetVersions(c *gin.Context) {
	...
	err = tenantDB.Query(c, `
		...
		SELECT vc.id, vc.version, vc.name, vc.file_size, vc.created_at, COALESCE(u.first_name || ' ' || u.last_name, u.first_name, u.last_name, '') as uploaded_by_name
...
func (h *DocumentHandler) ListFirm(c *gin.Context) {
	...
	query.WriteString(`
		SELECT d.id, d.name, d.original_name, d.file_size, d.mime_type, d.status, d.access,
		       d.created_at, d.updated_at, COALESCE(u.first_name || ' ' || u.last_name, u.first_name, u.last_name, '') as uploaded_by_name
		FROM documents d
		LEFT JOIN users u ON d.uploaded_by = u.id
